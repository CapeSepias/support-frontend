...
import { FulfilmentOptions } from './productPrice/fulfilmentOptions';
import './productPrice/fulfilmentOptions';
...
>;
export type FlashSaleSubscriptionProduct =
  | SubscriptionProduct
  | 'AnnualDigitalPack';
type Sale = {
...
const dpSale = {
  ...
  saleCopy: {
    featuredProduct: {
      ...
      description:
        'Read The Guardian ad-free on all devices, including the Premium App and UK Daily Edition iPad app.',
    },
    landingPage: {
      heading: 'Digital Pack subscriptions',
      subHeading:
        'Save 50% for 3 months on award-winning, independent journalism, ad-free on all of your devices',
      roundel: [''],
    },
    bundle: {
      heading: 'Digital Pack',
...
function getActiveFlashSales(
  ...
  const now = timeTravelDays
    ? Date.now() + timeTravelDays * 86400000
    : Date.now();
  const sales = Sales.filter(
    sale =>
      sale.subscriptionProduct === product &&
      sale.activeRegions.includes(countryGroupId),
  ).sort(sortSalesByStartTimesDescending);
  return sales.filter(
    sale =>
...
function getDiscount(
  ...
  return (
    ...
    sale.saleDetails[countryGroupId].discountPercentage
  );
}
...
function getIntcmp(
  ...
  if (flashSaleIsActive(product)) {
    const sale = getActiveFlashSales(product, countryGroupId)[0];
    return (
      (sale && sale.saleDetails[countryGroupId].intcmp) ||
      intcmp ||
      defaultIntcmp
    );
  }
...
function countdownTimerIsActive(
  flashSaleActive: boolean,
  showForHowManyDays: number,
  endTime: number,
): boolean {
  if (flashSaleActive) {
    const timeTravelDays = getTimeTravelDaysOverride();
    ...
    const now = timeTravelDays
      ? Date.now() + timeTravelDays * oneDayInMillis
      : Date.now();
