...
function bracketPromise<A, B>(
  acquire: () => Promise<A>,
  release: (arg0: A) => Promise<void>,
  use: (arg0: A) => Promise<B>,
): () => Promise<B> {
  const releaseAndReturn = (a: A) => (b: B): Promise<B> =>
    release(a).then(() => b);

  const releaseAndRethrow = (a: A) => (e: any): Promise<B> =>
    release(a).then(() => Promise.reject(e));

  return () =>
    acquire().then(a => use(a).then(releaseAndReturn(a), releaseAndRethrow(a)));
}
