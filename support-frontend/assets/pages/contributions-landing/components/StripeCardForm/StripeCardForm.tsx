...
import { PaymentResult } from 'helpers/paymentIntegrations/readerRevenueApis';
...
const CardForm = (props: PropTypes) => {
  ...
  const [calledRecaptchaRender, setCalledRecaptchaRender] = useState<boolean>(
    false,
  );
  ...
  const onChange = (fieldName: CardFieldName) => update => {
    ...
    setFieldStates(prevData => ({ ...prevData, [fieldName]: newFieldState() }));
  };
