...
import 'helpers/paymentIntegrations/readerRevenueApis';
import {
...
const mapStateToProps = (state: State, ownProps: PropTypes) => ({
  ...
  stripePaymentRequestButtonVariant:
    state.common.abParticipations.stripePaymentRequestButtonDec2020 === 'PRB',
...
function updateTotal(props: PropTypes) {
  ...
  if (
    ...
    props.stripePaymentRequestButtonData.stripePaymentRequestObject
  ) {
...
function onPayment(
  ...
  if (validatedCountryFromCard) {
    const validatedBillingStateFromCard: Option<StateProvince> = stateProvinceFromString(
