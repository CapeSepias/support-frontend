...
import { ExistingPaymentMethod } from 'helpers/existingPaymentMethods/existingPaymentMethods';
import {
  ...
  if (
    ...
    sendGetExistingPaymentMethodsRequest(
      ...
      (allExistingPaymentMethods: ExistingPaymentMethod[]) => {
        const switchedOnExistingPaymentMethods = allExistingPaymentMethods.filter(
...
const init = (store: Store<State, Action, (...args: Array<any>) => any>) => {
  ...
  selectInitialContributionTypeAndPaymentMethod(
    state,
    dispatch,
    contributionTypes,
  );
