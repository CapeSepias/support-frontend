...
// ---- Helpers ----- //
const getOfferStr = (
  subscription: Option<number>,
  newsstand: Option<number>,
): string => {
  if (
    subscription &&
    newsstand &&
    parseFloat(getNewsstandSaving(subscription, newsstand)) > 0
  ) {
    return `Save ${getNewsstandSavingPercentage(
      subscription,
      newsstand,
    )}% a month on retail price`;
  }
...
const mapStateToProps = (state: State): StateProps => ({
  ...
  products: state.page.productPrices
    ? getPlans(state.page.tab, state.page.productPrices)
    : [],
});
