...
function getRadioButtons(props: PropTypes) {
  return props.radios.map((radio: Radio, idx: number) => {
    ...
    return (
      <span
        id={radio.id}
        className={classNameWithModifiers('component-radio-toggle__button', [
          props.modifierClass,
        ])}
        key={radioId}
      >
        ...
        <label
          htmlFor={radioId}
          className={classNameWithModifiers('component-radio-toggle__label', [
            labelModifier,
          ])}
        >
          {radio.text}
...
export default function RadioToggle(props: PropTypes) {
  ...
  return (
    <div className="component-radio-toggle" aria-describedby={radioGroupId}>
      {radioButtons}
      <p id={radioGroupId} className="visually-hidden">
...
function A11yHint(props: { id: string; hint: string | null | undefined }) {
  return (
