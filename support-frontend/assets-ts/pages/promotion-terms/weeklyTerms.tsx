...
function getCountryPrice(
	...
	const quarterlyPrice =
		prices[fulfilmentOption][NoProductOptions][Quarterly][currency];
	const annualPrice =
...
function StandardCountryPrice(
	...
	const { quarterlyPrice, annualPrice } = getCountryPrice(
		nameAndSaving.name,
		prices,
	);
	return (
		<SansParagraph>
			<strong>{nameAndSaving.name}:</strong> Quarterly (13 weeks) subscription
			rate {showPrice(quarterlyPrice)}, or annual rate {showPrice(annualPrice)}
...
function SixForSixCountryPrice(
	...
	const { currencyGlyph, quarterlyPrice } = getCountryPrice(
		nameAndSaving.name,
		prices,
	);
	return (
		<SansParagraph>
			...
			first 6 issues followed by quarterly (13 weeks) subscription payments of{' '}
			{showPrice(quarterlyPrice)} thereafter{getSaving(nameAndSaving.saving)}
		</SansParagraph>
	);
}
