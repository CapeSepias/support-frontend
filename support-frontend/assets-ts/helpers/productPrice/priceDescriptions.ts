...
import { billingPeriodNoun as upperCaseNoun, billingPeriodAdverb, Quarterly, BillingPeriod } from 'helpers/productPrice/billingPeriods';
...
import { getAppliedPromo, hasDiscount, hasIntroductoryPrice, Promotion } from 'helpers/productPrice/promotions';
...
function getPriceDescription(productPrice: ProductPrice, billingPeriod: BillingPeriod, compact: boolean = false, useExtendedGlyph: boolean = true): string {
  ...
  if (hasDiscount(promotion)) {
    return getDiscountDescription(
      glyph, productPrice.price, productPrice.fixedTerm, // $FlowIgnore -- We have checked this in hasDiscount
